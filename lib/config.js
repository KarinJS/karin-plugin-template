...
class Config {
  ...
  /** 初始化配置 */
  initCfg () {
    let path = `${dirname}/config/config/`
    let pathDef = `${dirname}/config/defSet/`
  ...
  get package () {
    ...
    this._package = JSON.parse(fs.readFileSync(dirname + '/package.json', 'utf8'))
   ...
   */
  getYaml (type, name) {
    let file = `${dirname}/config/${type}/${name}.yaml`
    let key = `${type}.${name}`
