...
export const sendMsg = karin.command(/^#测试主动消息$/, async (e) => {
  ...
  /** 发送消息 */
  const { messageId } = await karin.sendMsg(e.selfId, contact, message, { recallMsg: 10 })
  /** 打印返回的消息ID */
...
export const forwardMessage = karin.command(/^#测试转发$/, async (e) => {
  ...
  /** 构建转发消息体 */
  const content = common.makeForward(message, e.selfId, e.karin.account.name)
  /** 发送转发消息 */
  await e.karin.sendForwardMessage(e.contact, content)
